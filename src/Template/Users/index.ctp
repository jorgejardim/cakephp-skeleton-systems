                ...
                <nav role="navigation" class="navbar navbar-default navbar-page-actions">
                    ...
                    <div id="navbarCollapseActionsPage" class="collapse navbar-collapse">
                        <ul class="nav navbar-nav">
                            <li><a href="<?= $this->Url->build(['action' => 'add', '?' => @$this->request->query]) ?>" class="btn-page-action"><i class="fa fa-plus-circle"></i> <?= __('Add New {0}', [__('User')]); ?></a></li>
                        </ul>
                    </div>
                </nav>
            </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-hover table-nomargin table-striped table-bordered">
                                <?php if($users->count()) { ?>
                                    ...
                                    <tbody>
                                        <?php
                                            $status = array(__('Blocked'), __('Active'));
                                        ...
                                        <?php foreach ($users as $user): ?>
                                            <tr>
                                                <td><?= h($user->name) ?></td>
                                                <td><?= h($user->email) ?></td>
                                                <td><?= $user->has('role') ? $this->Html->link($user->role->name, ['controller' => 'Roles', 'action' => 'edit', $user->role->id]) : '' ?></td>
                                                <td class="<?= $colors[$user->status] ?>"><?= $status[$user->status] ?></td>
                                                <td class="actions">
                                                    ...
                                                    <?= $this->Form->postLink('<i class="fa fa-trash-o"></i>', ['action' => 'delete', $user->id, '?' => @$this->request->query], ['confirm' => __('Are you sure you want to delete # {0}?', $user->id), 'title' => __('Delete'), 'class' => 'btn btn-red btn-listing btn-small', 'escape' => false, 'data-toggle'=>'tooltip', 'data-placement'=>'top']) ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
