...
class CommonsHelper extends Helper
{
    ...
    public function menu_link($link) {
        if($link=='#') {
            return $link;
        } elseif($link=='/' || !$link) {
            return $this->Url->build('/', true);
        } elseif($link[0]!='/') {
            return $this->Url->build('/p/'.$link, true);
        }
        return $this->Url->build($link, true);
    }

    public function menu_current_link($link) {
        if($this->menu_link($link) == $this->Url->build()) {
            return true;
        }
        return false;
    }

    public function player_link($link) {
        ...
        if (preg_match('/vimeo/', $link)) {
            ...
            return 'http://player.vimeo.com/video/'.$matches[2].'?color=ffffff&title=0&byline=0&rel=0&portrait=0';
        }
        return $link;
    }
