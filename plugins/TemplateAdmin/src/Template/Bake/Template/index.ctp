...
%>
                ...
                <nav role="navigation" class="navbar navbar-default navbar-page-actions">
                    ...
                    <div id="navbarCollapseActionsPage" class="collapse navbar-collapse">
                        <ul class="nav navbar-nav">
                            <li><a href="<?= $this->Url->build(['action' => 'add', '?' => @$this->request->query]) ?>" class="btn-page-action"><i class="fa fa-plus-circle"></i> <?= __('Add New {0}', [__('<%= Inflector::humanize($singularVar) %>')]); ?></a></li>
                        </ul>
                    </div>
                </nav>
            </div>
        ...
        <div class="breadcrumbs">
            <ul>
                ...
                <li>
                    <?= $this->Html->link(__('List {0}', [__('<%= Inflector::humanize($pluralVar) %>')]), ['action' => 'index', '?' => @$this->request->query]); ?>
                </li>
            </ul>
        </div>
        <div class="row">
            <div class="col-sm-12">
                <div class="box">
                  ...
                  %>
                  <td class="actions">
                                                    <?= $this->Html->link('<i class="fa fa-eye"></i>', ['action' => 'view', <%= $pk %>, '?' => @$this->request->query], ['title' => __('View'), 'class' => 'btn btn-blue btn-listing btn-small', 'escape' => false, 'data-toggle'=>'tooltip', 'data-placement'=>'top']) ?>
                                                    ...
                                                    <?= $this->Form->postLink('<i class="fa fa-trash-o"></i>', ['action' => 'delete', <%= $pk %>, '?' => @$this->request->query], ['confirm' => __('Are you sure you want to delete # {0}?', <%= $pk %>), 'title' => __('Delete'), 'class' => 'btn btn-red btn-listing btn-small', 'escape' => false, 'data-toggle'=>'tooltip', 'data-placement'=>'top']) ?>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
